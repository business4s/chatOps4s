...
/** Storage for the current config-token / refresh-token pair.
  ...
  * ==HA limitations==
  *
  * Refresh tokens are '''single-use''': each call to `tooling.tokens.rotate` invalidates the previous refresh token. The in-memory implementation is
  * therefore single-process only.
  *
  * For high-availability deployments, implement this trait with atomic compare-and-swap semantics (e.g. Redis `WATCH`/`MULTI` or a database row with
  * a version column). When two processes race to rotate, the loser's refresh token will already be invalid; it should re-read the store to pick up
  * the winner's tokens.
  */
