...
class BlocksDeserializationTest extends AnyFreeSpec with Matchers {
  ...
  "Block serialization" - {
    ...
    "SectionBlock with fields" in {
      val json = encodeBlock(SectionBlock(
    ...
    "SectionBlock with static_select accessory and options" in {
      val json = encodeBlock(SectionBlock(
        ...
        accessory = Some(StaticSelectElement(
          action_id = "text1234",
    ...
    "SectionBlock with radio_buttons accessory" in {
      ...
      val json = encodeBlock(SectionBlock(
        ...
        accessory = Some(RadioButtonGroupElement(
    ...
    "InputBlock with plain_text_input and hint" in {
      val json = encodeBlock(InputBlock(
    ...
    "ContextBlock with mixed elements" in {
      val json = encodeBlock(ContextBlock(
        elements = List(
    ...
    "ImageBlock with image_url" in {
      val json = encodeBlock(ImageBlock(
        alt_text = "A photo",
    ...
    "ImageBlock with slack_file" in {
      val json = encodeBlock(ImageBlock(
        ...
        slack_file = Some(SlackFileObject(
          id = Some("F111111"),
    ...
    "VideoBlock" in {
      val json = encodeBlock(VideoBlock(
  ...
  "Element serialization" - {
    ...
    "multi_static_select with options" in {
      val json = encodeElement(MultiStaticSelectElement(
        action_id = "text1234",
    ...
    "conversations_select with filter" in {
      val json = encodeElement(ConversationsSelectElement(
        action_id = "conv",
        filter = Some(ConversationsFilter(
    ...
    "button with confirmation dialog" in {
      val json = encodeElement(ButtonElement(
    ...
    "static_select with option_groups" in {
      val json = encodeElement(StaticSelectElement(
        action_id = "grouped",
        option_groups = Some(List(
          OptionGroupObject(
